...
if (isset($_COOKIE['adminCookie']) and adminCookieExists($config['sqlUname'], $config['sqlPasswd'], $config['sqlDB'], preg_replace("/[^0-9.]+/i", "", $_COOKIE['adminCookie']))) {
    ...
    if ($output[0] == 1) {
        echo "Something went wrong with deleting the user!<br><button onclick=\"AJAX('/accountTools/student', 'mainEmbed')\" >Return home</button>";
        exit();
    }
    echo "Success! User deleted!<br><button onclick=\"AJAX('/accountTools/student', 'mainEmbed')\" >Return home</button>";
    exit();
} else {
