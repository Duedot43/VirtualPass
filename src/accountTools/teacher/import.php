...
if (isset($_COOKIE['adminCookie']) and adminCookieExists($config['sqlUname'], $config['sqlPasswd'], $config['sqlDB'], preg_replace("/[^0-9.]+/i", "", $_COOKIE['adminCookie']))) {
    ...
    if (isset($_FILES["csv"])) {
        ...
        foreach ($teachers[1] as $teacherRecord) {
            $output = sendSqlCommand("INSERT teachers VALUES('" . $teacherRecord['uname'] . "', '" . $teacherRecord['passwd'] . "', '" . $teacherRecord['uuid'] . "')", $config['sqlUname'], $config['sqlPasswd'], $config['sqlDB']);
            if ($output[0] != 0) {
...
<tr>
    <form method="post" name="form" enctype="multipart/form-data" action="/accountTools/teacher/import.php">
        <td>
